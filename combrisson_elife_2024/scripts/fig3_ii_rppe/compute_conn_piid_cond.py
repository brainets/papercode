...
if __name__ == '__main__':
    ...
    # i/o settings
    to_folder = f'conn/piid/%s-%s/%s/cond'
    ###########################################################################
    ...
    parser.add_argument('--relation', default='intra', type=str,
                        ...
                        help='ROI relation')
    parser.add_argument('--band', default='bga', type=str,
    ...
    parser.add_argument('--signit', default=1, type=int, choices=[0, 1],
                        help='Significance testing (1) or CI (0)')
    ...
    for n_s, s in enumerate(subjects):
        ...
        for cond in ['rew', 'pun']:
            # condition selection
            logger.info(f'    - Select {cond} condition')
            _da_c = _da.isel(model=[o == cond for o in out['cond']])

            # copnorm the data
            logger.info('    - Copnorm the data')
            _da_c.data = copnorm_nd(_da_c.data, axis=0)
            if mi_type == 'cc':
                _da_c['model'] = copnorm_nd(_da_c['model'].data, axis=0)

            _ii[cond], _ii_p[cond] = compute_conn(
                _da_c, mi_type, metric, n_perm, -1, hemi, hemi_links,
                relation
            )
        ii.append(xr.Dataset(_ii).to_array('cond'))
        ii_p.append(xr.Dataset(_ii_p).to_array('cond'))
    ...
    ii_rew = [k.sel(cond='rew').data for k in iir]
    ii_p_rew = [k.sel(cond='rew').data for k in iir_p]
    _outs = compute_stat(
        'rew', ii_rew, ii_p_rew, times, roi, **kw_stats
    )
    dt.update(**_outs)
    ...
    ii_pun = [k.sel(cond='pun').data for k in iir]
    ii_p_pun = [k.sel(cond='pun').data for k in iir_p]
    _outs = compute_stat(
        'pun', ii_pun, ii_p_pun, times, roi, **kw_stats
    )
    dt.update(**_outs)

    # _______________________________ REW > PUN _______________________________
    kw_stats['tail'] = 1

    set_log_level(True)
    logger.info("        Stats on REW > PUN")
    ii_rewpun = [r - p for r, p in zip(ii_rew, ii_pun)]
    ii_p_rewpun = [r - p for r, p in zip(ii_p_rew, ii_p_pun)]
    _outs = compute_stat(
        'rew>pun', ii_rewpun, ii_p_rewpun, times, roi, **kw_stats
    )
    dt.update(**_outs)

    # _______________________________ PUN > REW _______________________________
    set_log_level(True)
    logger.info("        Stats on PUN > REW")
    ii_punrew = [p - r for r, p in zip(ii_rew, ii_pun)]
    ii_p_punrew = [p - r for r, p in zip(ii_p_rew, ii_p_pun)]
    _outs = compute_stat(
        'pun>rew', ii_punrew, ii_p_punrew, times, roi, **kw_stats

    )
    dt.update(**_outs)

    # ______________________________ REW != PUN _______________________________
    kw_stats['tail'] = 0
    logger.info("        Stats on REW != PUN")
    ii_rewpun = [r - p for r, p in zip(ii_rew, ii_pun)]
    ii_p_rewpun = [r - p for r, p in zip(ii_p_rew, ii_p_pun)]
